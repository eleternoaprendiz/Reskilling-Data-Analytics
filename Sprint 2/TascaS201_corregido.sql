...
SELECT 
    ...
    c.website 'Página web',
    COUNT(t.id) 'Núm. trans.',
    ROUND(MAX(t.amount)) 'Transacción máxima',
    ROUND(AVG(t.amount)) 'Media transacciones'
FROM
    company c
JOIN
    transaction t ON t.company_id = c.id
WHERE
    c.id IN (SELECT company_id
			 FROM transaction
			 WHERE amount > (SELECT AVG(amount)
							 FROM transaction))
GROUP BY t.company_id
ORDER BY País, Empresa;
